...
**直接用线上网页即可，无需任何配置。** 浏览器打开 <https://manu6lenses.win> 就能开始探索。网站托管在 Cloudflare，完全免费，不需要 API key 或本地安装。

本地开发（以及其对应的 API key）仅在你想**自行运行和修改代码**时才需要，详见下方「本地运行」。
...
> 这仅适用于**本地开发代码**的场景。普通访客无需任何操作——直接用上面的线上网址即可。下面的 API key 只在本地测试时由服务端读取，网页上永不出现。
